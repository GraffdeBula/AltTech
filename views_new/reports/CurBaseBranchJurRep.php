...
<html>
    <head>
        <meta charset="UTF-8">
        <title>Действующая база клиентов. Юридическая стадия</title>       
    </head>
    <body>
        <div>            
            <form method="get">
                <?php (new MyForm('CurBaseBranchJurCtrl','ShowBrBase',0,0))->AddForm2();
                ...
                <div class="tab-pane fade active show" id="repbase">
                    <?php
                        if ((isset($_GET['Branch']))&&($_GET['Branch']!='')){
                            echo("<a href='/".WORK_FOLDER."/downloads/Действующая база ЮС {$_GET['Branch']}.xlsx'><button class='btn btn-success'>Выгрузить в Excel</button></a>");
                        }
                    ?>
