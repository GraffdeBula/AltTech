...
class Payment {    
    ...
    public function getPayData(){//метод для получения данных о платеже (не важно откуда)
        //новый платёж получает данные из Get запроса
        
        //уже сделанный платёж получает данные из Базы данных
    ...
    public function formPayBill($Id,$ContCode) {
        $this->getPaymentById($BillNum);
        if (($this->PayType==2) or ($this->PayType==9) or ($this->PayType==12)){            
            $this->printReturn();
        } else {            
            $this->printPayment();
